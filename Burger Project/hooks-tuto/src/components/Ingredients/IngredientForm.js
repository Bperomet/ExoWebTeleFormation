...
const IngredientForm = React.memo(props => { // react.memo pour eviter de refaire le composent 

  const inputState = useState({
    title: '',
    amount: ''
  });
  //peut avoir plusieurs fois states
  const [inputStateEmail, setInputStateEmail] = useState({
    email: ''
  });
 /*
*pour recuperer les 2 elements du tableau
        ...
        <form onSubmit={submitHandler}>
          ...
          <div className="form-control">
            ...
            <input type="number" id="amount"  value={inputState[0].amount} onChange={event=>{
              ...
              inputState[1]((prevInputState)=>({amount: newNumber, title: prevInputState.title}))}}/>
          </div>
